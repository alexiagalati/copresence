...
#OK, let's focus just on the DV ofinitial words (wordsin) in the descriptions.
...
#We'll check that later, once we build the models. 
...
#All are within-participant factors

#BEFORE BUILDING A MAXIMAL MODEL, LET'S BUILD SOME PRELIMINARY MODELS, 
#FOLLOWING THE BAAYEN APPROACH TO GAIN A BETTER UNDERSTANDING OF LMERs
...
#That variability reflects the fact that every description has some factors that affect the description's length (in terms of initial words) that are not captured by this experiment.
...
#Using the print_stats function, we can get the associated p-values from an F-distribution (I got this from Rick Dale).
#Bodo Winters in his tutorials describes an alernative approach for getting p-values (through model comparison). [http://www.bodowinter.com/tutorial/bw_LME_tutorial2.pdf]
...
#although it's possible that speakers will keep track of co-presence distinctions with B when they've interacted with B consecutively (this prediction is about the interaction between partner * partner order, actually). 

#Let's build the model, and then compare it to model 1, to examine if it improves the fit of the data.
...
#As you can see in the outpout, each director and item have different intecepts. This makes sense, since directors and items are specified as random effects in the model.
...
#OK, so we've established that makes sense to have random slope models, and we've already practiced building a few theoretically motivated random slope models following the Baayen et al. approach.
...
#WARNING: This model might take a while to run! 
...
#A final approach that I've seen elsewhere is to remove the higher order terms (interactions) in the random effect structure, e.g., simplifying partnero*partner into partnero + partner
